...
> 📝 DNS-серверы, привязанные к туннельным интерфейсам (`ip name-server ... on ...`), можно оставить.
...
# Туннельные интерфейсы для зарубежного DNS (защита от MITM)
OTHER_DNS_INTERFACES=""

# Туннельный интерфейс для DNS зоны (обычно не нужен)
ZONE_DNS_INTERFACE=""

# Политика шифрования DNS
DNS_TRANSPORT="auto"
```

#### DNS_TRANSPORT — политика шифрования

| Значение | Описание |
|----------|----------|
| `auto` (по умолч.) | DoT/DoH preferred, UDP fallback для zone-провайдеров с `UDP_FALLBACK=yes` |
| `strict` | Только DoT/DoH, без UDP fallback. Весь DNS зашифрован |

- **`auto`** — рекомендуется для большинства. Зоновые провайдеры (Yandex, AliDNS) используют DoT с UDP-фолбеком для надёжности. International провайдеры всегда через DoH.
- **`strict`** — весь DNS-трафик гарантированно зашифрован. Если DoT/DoH-сервер недоступен, запрос не будет перенаправлен на UDP. Подходит для сетей с DPI-инспекцией.
...
> 📝 Строка **Config** показывает количество серверов, правил и пользовательских провайдеров (custom). Строка **Other tunnels** отображается только если настроены `OTHER_DNS_INTERFACES`.
...
| Зарубежные домены не резолвятся | DoH фильтруется на сетевом уровне | Включить туннельный режим (см. ниже) |
...
## Интеграция с туннелями (опционально)

Если зарубежные DNS недоступны напрямую (DoH фильтруется MITM), направьте DNS через шифрованный канал:

### Настройка

1. Узнать имена туннельных интерфейсов:
   ```sh
...
3. Указать туннельные интерфейсы для international DNS:
   ```sh
...
SmartDNS отправляет DNS-запросы к международным серверам через указанные туннельные интерфейсы параллельно. Первый ответ побеждает. Если все туннели недоступны — автоматический fallback на прямое соединение.

### Пример: два туннеля для redundancy

```sh
OTHER_DNS_INTERFACES="nwg3 nwg4"
```

- `nwg3`: DNS через туннель 1
- `nwg4`: DNS через туннель 2
- Fallback: напрямую (без туннеля)
