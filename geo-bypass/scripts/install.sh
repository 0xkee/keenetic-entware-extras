...
create_init_script() {
  local init_script="/opt/etc/init.d/S99geo-bypass"

  cat > "$init_script" << INITEOF
#!/opt/bin/sh
# geo-bypass startup script — auto-generated by install.sh
SCRIPTS="$INSTALL_DIR/scripts"
CONFIG="$INSTALL_DIR/config/config.sh"
...
case "\$1" in
  ...
  restart)
    ...
    ;;
  status)
    if "\$SCRIPTS/status.sh" 2>/dev/null; then
      :
    else
      echo "geo-bypass: status script not available"
    fi
    ;;
  *)
