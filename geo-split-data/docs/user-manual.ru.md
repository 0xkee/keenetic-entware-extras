...
**232 зоны** — агрегированные подсети всех стран мира (загружены при сборке пакета из ipdeny.com):

| Файл | Страна | Примерный объём |
|------|--------|-----------------|
| `ru.zone` | 🇷🇺 Россия | ~13 000 CIDRs |
| `by.zone` | 🇧🇾 Беларусь | ~1 200 CIDRs |
| `kz.zone` | 🇰🇿 Казахстан | ~1 500 CIDRs |
| `cn.zone` | 🇨🇳 Китай | ~8 500 CIDRs |
| `us.zone` | 🇺🇸 США | ~70 000 CIDRs |
| `de.zone` | 🇩🇪 Германия | ~10 000 CIDRs |
| … | | 232 зоны всего |
...
> 📝 Подсети объединяются в один маршрутный набор. Файлы нужны как seed при первом запуске — в дальнейшем geo-split может обновлять данные из интернета по расписанию.
