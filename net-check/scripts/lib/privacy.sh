...
privacy_filter() {
  ...
  # --- Step 1: Targeted replacements from geo cache ---
  local _n=0
  for _gf in "${DATA_DIR}"/geo-*.json; do
    ...
    if [ -n "$_gcc" ] && [ ${#_gcc} -eq 2 ]; then
      ...
      {
        # JSON: "country":"NL" → "country":"JP"
        printf 's/"country":"%s"/"country":"%s"/g\n' "$_gcc" "$_fake_cc"
        # Table cell: " CC " (space-bounded, CC:4 columns)
        printf 's/ %s / %s /g\n' "$_gcc" "$_fake_cc"
        # Table header: "(CC)" — cmp_header "iface (CC)" and CDN "→ IP (CC)"
        printf 's/(%s)/(%s)/g\n' "$_gcc" "$_fake_cc"
        # Multi-CC in parens: "(CC1 CC2)" — first CC followed by space
        printf 's/(%s /(%s /g\n' "$_gcc" "$_fake_cc"
        # Multi-CC in parens: "( CC2)" — space + last CC + close paren
        printf 's/ %s)/ %s)/g\n' "$_gcc" "$_fake_cc"
