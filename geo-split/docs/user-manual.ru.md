...
| Сценарий | Описание |
|----------|----------|
| 🌐 Региональные → ISP | Региональные подсети идут через провайдера, остальное — через туннель (например, ЕАЭС, ЕС, ASEAN — 232 зоны) |
...
> 💡 `"auto"` — рекомендация для типичного сценария раздельной маршрутизации.
