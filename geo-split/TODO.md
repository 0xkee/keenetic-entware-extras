...
- [x] **CIDR support в route-check** — гибридный подход: sampling 1-3 IPs + coverage analysis из geo-split таблиц. `is_cidr()`, `cidr_sample_ips()`, `cidr_total_ips()` в lib/ip.sh, `_cidr_overlap_routes()` в route-check.sh, `validate_cidr()` в api-router.lua, CIDR пример в WebUI. JSON включает `input_type`, `coverage` (overlaps, geo_split_pct). **(v0.17.0, 2026-07-07)**
