...
### geo-bypass: route-based (без fwmark)
...
```
Это **отключает аппаратное ускорение NAT** для всей сети, что может снизить throughput роутера. geo-bypass этого не требует.
...
**keen-pbr** здесь сильнее: dnsmasq-интеграция даёт **мгновенный** domain routing при первом же DNS-запросе. geo-bypass опирается на cron (15 мин) + dig, что создаёт задержку для новых IP.

Однако geo-bypass **не подменяет dnsmasq**, а использует отдельный `dig` — что безопаснее в плане «не сломать DNS на роутере».
...
geo-bypass использует стандартные `ip route show default` / `ip -o link show up` — проще, но менее точно.
...
- **geo-bypass**: S99 (последний старт) — все интерфейсы гарантированно up → сразу рабочий маршрут, но окно ~1-3 сек при boot без маршрутов
...
```mermaid
flowchart TB
  subgraph GB["geo-bypass (POSIX sh)"]
    direction TB
...
keen-pbr не тестировался на 13K+ записях — другой use case. geo-bypass оптимизирован именно для country-level объёмов.

---

## 7. Что есть в keen-pbr, чего нет в geo-bypass
...
## 8. Что есть в geo-bypass, чего нет в keen-pbr
...
### Почему geo-bypass не использует fwmark
...
geo-bypass использует `ip rule iif br0` + per-subnet routes, что **не касается mark** и поэтому совместимо с Keenetic NDM routing.
...
1. ✓ **Kill switch** — blackhole route в table 1000 при падении VPN (для `vpn` mode)
   ...
   - Для geo-bypass: `ip route add blackhole default table 1000 metric 200` + логика удаления default route в `ndm-hook.sh` при VPN down
...
6. ✓ **Multiple ipsets** — разные списки → разные VPN/interfaces
   ...
   - ✓ Может быть реализовано как множество instansов geo-bypass
...
**keen-pbr** и **geo-bypass** решают пересекающуюся, но не идентичную задачу с **фундаментально разными архитектурными подходами**:
...
Проекты **не являются конкурентами** — это разные инструменты для разных задач. keen-pbr — universal PBR toolkit; geo-bypass — специализированный GEO-routing, оптимизированный под Keenetic NDM.

**Главное преимущество geo-bypass**: полная совместимость с Keenetic per-device routing (fwmark-free) без деградации производительности (HW NAT сохранён).
