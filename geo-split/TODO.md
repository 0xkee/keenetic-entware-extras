...
- [x] **`--from` для route-check CLI** — `--from <MAC>` резолвит клиент → fwmark через iptables mangle HOTSPOT + имя через ndmc hotspot. Non-VPN клиенты корректно показывают default (без auto-detect чужого fwmark). Fuzzy-поиск по имени — отложен. **(v0.16.0, 2026-06-30)**
- [ ] **CIDR support в route-check** — поддержка проверки подсетей (напр. `10.0.0.0/24`) в CLI и WebUI. Требуется: `is_cidr()` в `lib/ip.sh`, разрешить `/` в `validate_host()` (api-router.lua), ветка в `route-check.sh` — skip DNS, `ip route get <first_host>`. Вопрос: проверять 1 IP (network addr) или sample из нескольких? Для /24=254 хоста — проверка всех дорогая. Предложение: sample first+last+mid (3 проверки), показать mixed если разные пути.
