...
- S39 — после S38smartdns (SmartDNS уже запущен), до S60smartdns и S99geo-bypass
...
- [ ] Проверить: как `ndm-hook.sh` (geo-bypass) взаимодействует с DNAT? При interface down/up DNAT правила могут сброситься?
...
### 2.4 Интеграция SmartDNS ↔ geo-bypass ipset

**Текущее:** geo-bypass `update-domains.sh` резолвит домены через `dig @localhost` → добавляет IP в ipset.

**Возможность SmartDNS:** SmartDNS умеет добавлять resolved IP напрямую в ipset!

```conf
# Добавлять resolved IP из группы ru в ipset geo-bypass
ipset /ru/geo-bypass
# Или для конкретных доменов:
# ipset /gosuslugi.ru/geo-bypass
```

- [ ] Исследовать: заменит ли это `update-domains.sh` полностью?
- [ ] Исследовать: `ipset` vs `nftset` directive в SmartDNS
- [ ] Исследовать: как это взаимодействует с geo-bypass ipset (атомарный swap)?
...
│   └── status.sh              # НОВЫЙ: диагностика DNS (как geo-bypass/status.sh)
├── .project/
│   ├── target-arch.md         # обновить (DNAT redirect, интеграция с geo-bypass)
...
- **geo-bypass** — зависит от SmartDNS для `dig @localhost` в `update-domains.sh`
