# Исследование: готовые альтернативы geo-bypass для Entware/Keenetic
...
Существует ли готовый пакет для Entware (типа `opkg install geo-routing`), который делает то же самое, что наш `geo-bypass`?

## Что делает geo-bypass
...
- Можно использовать как source URL для `geo-bypass`
...
Наш `geo-bypass` использует route-based подход (без fwmark) именно потому, что fwmark ломает Keenetic NDM. Адаптация `pbr` потребовала бы полной переделки его routing-механизма.

### Количественная оценка

| Метрика | Адаптация `pbr` | Наш `geo-bypass` |
|---------|----------------|-------------------|
...
| Доп. возможности сверх geo-bypass | per-port, per-source IP routing | редко нужно |

### Что `pbr` умеет, а `geo-bypass` — нет
...
3. **Per-domain routing** через dnsmasq nftset  
   → Частично реализовано в `geo-bypass` через `dig` + ipset
4. **Web UI** (LuCI)  
   → Отдельная задача, не зависящая от `pbr`

---

## Сравнение: geo-bypass vs альтернативы

| Аспект | geo-bypass | ruantiblock | OpenWrt `pbr` | Keenetic встроенный |
|--------|-----------|-------------|---------------|-------------------|
...
1. **Готового пакета для Entware нет** — в `opkg` отсутствует аналог geo-bypass
2. **Адаптация `pbr` нецелесообразна** — 86% кода нужно переписать, fwmark фундаментально несовместим с Keenetic
3. **`geo-bypass` архитектурно правильнее** для Keenetic, чем любое существующее решение, потому что:
   - Route-based подход без конфликтов с NDM
   - NDM hooks для реакции на интерфейсы
   - Минимальные зависимости (только Entware `ip-full`)
4. **При необходимости расширения** (per-device, per-domain) — проще добавлять фичи в `geo-bypass`, чем портировать чужое решение с несовместимой архитектурой
